  ...
  const onsubmit = async (data: Data) => {
    ...
    try {
      const response = await axios.post(
        'http://127.0.0.1:7000/transcendence/auth/signin/local',
        sendData,
      );
      ...
      {
        userInfo &&
          location(`/transcendence/user/profile`, {
  ...
  const getUserInfo = async () => {
    try {
      const response = await axios.get(
        `http://localhost:7000/transcendence/user/by-token/${sessionStorage.getItem(
          'refresh_token',
        )}`,
        {
          headers: {
    ...
    <>
      <div className="backdrop-blur-md p-0 flex-row lg:px-4 xl:px-16 bg-black/50 min-w-full min-h-full z-[668] absolute flex justify-between bg-clip-padding">
        ...
        <div className="flex flex-col justify-center items-center forms min-w-full min-h-screen lg:min-w-fit lg:min-h-fit">
          <Form onSubmit={onsubmit} render={FormLogin}></Form>
        </div>
      </div>
      <Background />
    </>
